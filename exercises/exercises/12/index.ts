...
Вступление:
    ...
    У нас так много пользователей и администраторов в базе данных!
    Отец генерального директора Джефф говорит, что мы сейчас занимаемся большими данными
    стартап. Мы понятия не имеем, что это значит, но
    Джефф говорит, что нам нужно провести некоторую статистику и
аналитику.

    Мы провели опрос внутри команды, чтобы
выяснить, что мы знаем о статистике.
    Единственным человеком, который заполнил его, был
специалист по обслуживанию наших кофемашин. Ответы были следующими:
    ...
    Мы нашли фрагмент кода в stackoverflow и
    скомпилировал его в модуль `статистика`. Плохо
то, что в нем отсутствуют объявления типов.

Упражнение:

    Проверьте реализацию модуля статистики по адресу:
...
Бонусное упражнение повышенной сложности:
    ...
    Избегайте дублирования объявлений типов.
