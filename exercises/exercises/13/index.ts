...
Вступление:
    ...
    Следующим логичным шагом для нас является предоставление более
    точной даты регистрации для наших пользователей и администраторов.
    Мы приблизительно определили даты для каждого пользователя и
    администратора и использовали библиотеку под названием "мастер дат",
    чтобы красиво отформатировать даты.

    К сожалению, объявления типов, которые поставлялись с
    библиотекой "date-wizard", были неполными.

    1. Отсутствует интерфейс сведений о дате
       поля, связанные со временем, такие как часы, минуты и
    секунды.
    2. Функция "pad" экспортирована, но не объявлена.

Упражнение:

    Проверьте реализацию модуля date-wizard в:
    node_modules/date-wizard/index.js
    node_modules/date-wizard/index.d.ts

    Расширьте объявление типа этого модуля в:
